...
| Evidence | 用命令输出和 gate 结果支撑“完成了”的结论；结构化 Eval 仍在演进 |
...
| Evidence | 当前 gate output 与验证证据；后续演进为结构化 Eval run | pipeline output、AC coverage、drift diagnostics |
...
- `pipeline --json-out`、eval run schema 和趋势指标；

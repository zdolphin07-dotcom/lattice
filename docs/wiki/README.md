...
当前实现已经具备最小可信闭环：安装、初始化、PrismSpec 引导、目录化 spec、per-spec context、基础 context 供给、验证 pipeline、基础 gates、结构化 eval run、Markdown summary 和 smoke test。尚未完成的是 review/TDD 语义证据、插件协议和多语言 drift parser。
...
flowchart LR
    ...
    H --> EV["Eval JSON / Markdown Summary"]
    EV --> F["summary.md"]
    F --> L["Learn Draft"]
    L -. durable lessons .-> K
```
...
- `pipeline --json-out`、eval run 数据集和 Markdown summary。

...
当前实现已经具备最小可信闭环：安装、初始化、PrismSpec 引导、目录化 spec、per-spec context、基础 context 供给、验证 pipeline、基础 gates、结构化 eval run、review/TDD process evidence、Markdown summary 和 smoke test。尚未完成的是跨 run 趋势、插件协议和多语言 drift parser。
...
flowchart LR
    ...
    H --> EV["Eval JSON / Process Evidence / Markdown Summary"]
    EV --> F["summary.md"]
    F --> L["Learn Draft"]
    L -. durable lessons .-> K
```
...
- `review-summary.json`、`tdd-evidence.json`、eval run 数据集和 Markdown summary。

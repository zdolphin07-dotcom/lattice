...
| Evidence 已有 pipeline/gate JSON、loop state、failure category、escalation learn draft、process evidence、Markdown summary、history report、CI artifact 与 PR comment 模板，但 promotion 仍弱 | 候选经验可能停留在 drafts | learn draft promotion workflow |

### P1：Evidence 与 Loop

| Gap | 影响 | 建议 |
|-----|------|------|
| failure category rules 还在 pipeline 内部 | 团队难按自定义 gate 扩展分类 | configurable failure category schema |
...
- configurable failure category schema。
...
2. 增加 configurable failure category schema 与 learn draft promotion workflow。

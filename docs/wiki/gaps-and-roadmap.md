...
| Evidence 已有 pipeline/gate JSON、process evidence、Markdown summary、CI artifact 与 PR comment 模板，但跨 run 趋势仍弱 | review/TDD/人工检查难做趋势分析 | 推进 eval history aggregation |
...
| Dashboard/history 未沉淀 | 只能看单次运行，不能看团队质量趋势 | eval history aggregation |
...
2. 做 eval history aggregation 与 review/TDD trend metrics。

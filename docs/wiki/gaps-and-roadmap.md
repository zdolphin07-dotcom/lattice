...
| Evidence 仍是文本输出 | 难做趋势、复盘和 dashboard | 推进 `pipeline --json-out` 与 eval run schema |

### P1：Evidence 与 Loop

| Gap | 影响 | 建议 |
|-----|------|------|
| pipeline 无 JSON 输出 | 无法稳定进入 CI artifact 和趋势分析 | `pipeline.sh --json-out` |
| 无 run id/spec hash/git sha | 难复盘一次交付 | eval run schema |

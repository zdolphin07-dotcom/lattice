...
| Evidence 已有 pipeline/gate JSON 与 CI artifact 模板，但语义证据仍弱 | review/TDD/人工检查难进入趋势 | 推进 review JSON、TDD evidence、PR summary |
...
| eval summary 未生成 | artifact 需要人工下载阅读 | markdown summary / PR comment |
...
- eval markdown summary / PR comment contract。
- review verdict JSON。
- TDD evidence JSON。
...
2. 做 eval markdown summary / PR comment contract，让 PR 能直接展示 eval run。
